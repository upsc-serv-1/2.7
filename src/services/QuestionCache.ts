...
class QuestionCacheService {
  ...
  async searchLocal(query: string, mode: 'Matching' | 'Exact', fields: string[] = ['Questions', 'Explanations']): Promise<CachedQuestion[]> {
    ...
    // FUZZY FALLBACK: If results are sparse and term is long enough, try 1-char tolerance (match online standards)
    if (mode !== 'Exact' && keywords.length === 1 && term.length > 3) {
      ...
      for (let i = 0; i < word.length; i++) {
        ...
        fuzzyRegexes.push(new RegExp(pattern, 'i'));
      }

      for (const testId of testIds) {
        if (results.length >= 25) break; // Safety cap for fuzzy results
        const data = await AsyncStorage.getItem(`${QUESTION_CACHE_PREFIX}${testId}`);
        if (!data) continue;
        ...
        try {
          ...
          for (const q of questions) {
            ...
            const matchesFuzzy = fuzzyRegexes.some(re => re.test(text) || re.test(expl));
            if (matchesFuzzy) {
              results.push(q);
