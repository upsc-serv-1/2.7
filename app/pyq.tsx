...
export default function PyqAnalysisTab({ isEmbedded }: { isEmbedded?: boolean }) {
  ...
  const navigateToLearning = (opts: { subject?: string; subjects?: string[]; section?: string; sections?: string[]; micro?: string; micros?: string[]; year?: string; mode?: 'learning' | 'exam' }) => {
      ...
      <View style={[styles.panel, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>
          {distributionData.map(item => (
            ...
            >
            <Text style={[styles.filterChipText, { color: selSubjects.includes(item.name) ? '#FFFFFF' : '#475569' }]}>{item.name}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
          ...
          <>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>
              {allSections.map(sec => (
                ...
                >
                  <Text style={[styles.filterChipText, { color: selSections.includes(sec) ? '#FFFFFF' : '#475569' }]}>{sec}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          </>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>
              {allMicros.map(m => (
                <TouchableOpacity
                  ...
                  onPress={() => setSelMicros(prev => prev.includes(m) ? prev.filter(s => s !== m) : [...prev, m])}
                >
                  <Text style={[styles.filterChipText, { color: selMicros.includes(m) ? '#FFFFFF' : '#475569' }]}>{m}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          </>
          ...
          <View style={{ marginTop: 20, gap: 20 }}>
            {selSections.length === 0 && (
              <StickyHeatmapTable
                ...
                colors={colors}
                heatmapPalette={heatmapPalette}
                onCellPress={(section, year) => navigateToLearning({ subjects: selSubjects, section, year })}
              />
            )}
            ...
            <StickyHeatmapTable
              ...
              colors={colors}
              heatmapPalette={heatmapPalette}
              onCellPress={(micro, year) => navigateToLearning({ subjects: selSubjects, sections: selSections, micro, year })}
            />
          </View>
        )}
      </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: isEmbedded ? 'transparent' : colors.bg }]}>
      ...
      {selSubjects.length > 0 && !loading && (
        <View style={[styles.actionFab, { backgroundColor: colors.primary }]}>
          <TouchableOpacity 
            style={styles.actionFabPart} 
            onPress={() => navigateToLearning({ subjects: selSubjects, sections: selSections, micros: selMicros, mode: 'learning' })}
          >
            <Grid size={18} color="#fff" />
            <Text style={styles.actionFabText}>Learn</Text>
          </TouchableOpacity>
          <View style={[styles.actionFabDivider, { backgroundColor: 'rgba(255,255,255,0.3)' }]} />
          <TouchableOpacity 
            style={styles.actionFabPart} 
            onPress={() => navigateToLearning({ subjects: selSubjects, sections: selSections, micros: selMicros, mode: 'exam' })}
          >
            <TrendingUp size={18} color="#fff" />
            <Text style={styles.actionFabText}>Exam</Text>
          </TouchableOpacity>
        </View>
      )}
