...
export function QuickPracticeWidget({ colors }: { colors: any }) {
  const items = [
    { label: 'Random 10', icon: '≡ƒÄ▓', params: { mode: 'learning', subject: 'All', view: 'list' } },
    { label: 'PYQ Only', icon: '≡ƒô£', params: { mode: 'learning', subject: 'All', pyqMaster: 'PYQ Only', view: 'list' } },
    { label: 'Polity', icon: '≡ƒÅ¢∩╕Å', params: { mode: 'learning', subject: 'Polity', view: 'list' } },
  ];
  return (
    <View style={[ws.card, ws.full, { backgroundColor: colors.surface, borderColor: colors.border }]}>
      <View style={ws.cardHeader}>
