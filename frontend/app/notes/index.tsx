...
import {
  ...
  BookOpen, Search as SearchIcon, X, Grid3x3, List, ChevronLeft,
...
export default function NotesIndex() {
  ...
  const renderTile = ({ item: n }: { item: Node }) => (
    <TouchableOpacity
      style={[styles.tile, { backgroundColor: colors.surface, borderColor: colors.border, width: TILE_W }]}
      onPress={() => openNode(n)}
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      {visible.length === 0 ? (
        ...
        <FlatList
          ...
          numColumns={2}
          columnWrapperStyle={{ gap: COL_GAP, paddingHorizontal: 16 }}
          contentContainerStyle={{ paddingTop: 8, paddingBottom: 110, gap: COL_GAP }}
      ...
      {/* FAB */}
      <TouchableOpacity
        onPress={() => { setCreateType('folder'); setCreateOpen(true); }}
...
const styles = StyleSheet.create({
  ...
  toolRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 8 },
  ...
  // Tiles (grid)
  tile: { borderRadius: 18, borderWidth: 1, padding: 16, gap: 10, minHeight: 130 },
  ...
  // Rows (list) — generous spacing, NOT congested
  row: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 16, gap: 14, borderBottomWidth: StyleSheet.hairlineWidth },
