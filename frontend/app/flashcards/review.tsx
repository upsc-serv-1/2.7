...
export default function ReviewScreen() {
  ...
  const handleFlip = useCallback(() => {
    ...
    Animated.spring(flipAnim, {
      toValue: isFlipped ? 0 : 180, friction: 8, tension: 10, useNativeDriver: true,
    }).start();
  ...
  const nextCard = async () => {
    if (currentIndex < queue.length - 1) {
      flipAnim.setValue(0); setIsFlipped(false); setSelectedOption(null); setShowCorrect(false);
  ...
  const rate = async (grade: Grade) => {
    ...
    try {
      ...
      setIsFlipped(false); setShowCorrect(false); flipAnim.setValue(0);
      await nextCard();
    } catch (err: any) {
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: colors.bg }]}>
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            if (!isFlipped && !showCorrect) {
                              setSelectedOption(k); setShowCorrect(true);
        ...
        <View style={[styles.actions, { borderTopColor: colors.border }]}>
          {isFlipped ? (
            ...
            <TouchableOpacity style={[styles.showBtn, { backgroundColor: colors.primary }]} onPress={handleFlip} testID="btn-show-answer">
...
const styles = StyleSheet.create({
  ...
  cardTouch: { flex: 1 },
  card: { flex: 1, borderRadius: 32, padding: 32, borderWidth: 2, backfaceVisibility: 'hidden' },
