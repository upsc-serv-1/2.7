      ...
      if (sf.searchMode !== 'Exact' && (!data || data.length < 50) && term.length > 3) {
        ...
        if (words.length === 1) {
          ...
          if (fuzzyPatterns.length > 0) {
            let fuzzyQry = supabase.from('questions').select('id, question_text, explanation_markdown, subject, section_group, micro_topic, is_pyq, source, exam_group, exam_year, is_upsc_cse, is_allied, is_others, tests(institute, series, title)').limit(100);
