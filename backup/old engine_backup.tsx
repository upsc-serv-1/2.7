      ...
      while (from < MAX_TOTAL) {
        ...
        if (resIds && resIds.length > 0) {
          ...
          if (params.testId) {
            ...
            const stage = params.examStage || params.series;
        ...
        if (term && term.length > 3 && (!data || data.length < 50)) {
           ...
           if (words.length === 1) {
             ...
             let fuzzyQ = supabase.from('questions').select('*, tests(*)').or(fuzzyPatterns.join(',')).limit(100);
             // Re-apply same filters
             const insts = params.institutes || params.institute;
             if (insts && insts !== 'All') {
                const { data: tRows } = await supabase.from('tests').select('id').in('institute', insts.split(','));
                if (tRows) fuzzyQ = fuzzyQ.in('test_id', tRows.map(t => t.id));
             }
