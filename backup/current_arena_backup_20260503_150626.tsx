...
export default function UnifiedArenaSetup() {
  ...
  const fetchSearchResults = async () => {
    ...
    try {
      ...
      let query = supabase.from('questions').select('id, question_text, explanation_markdown, subject, section_group, micro_topic, is_pyq, is_ncert, source, exam_group, exam_year, is_upsc_cse, is_allied, is_others, tests(institute)').limit(100);
      ...
      if (sf.selectedSections?.length > 0) {
        ...
        if (sections.includes(null)) {
          ...
          if (nonNulls.length > 0) {
            query = query.or(`section_group.in.(${nonNulls.join(',')}),section_group.is.null`);
          }
      ...
      if (sf.searchMode !== 'Exact' && (!data || data.length < 50) && term.length > 3) {
        ...
        if (words.length === 1) {
          ...
          if (fuzzyPatterns.length > 0) {
            let fuzzyQry = supabase.from('questions').select('id, question_text, explanation_markdown, subject, section_group, micro_topic, is_pyq, is_ncert, source, exam_group, exam_year, is_upsc_cse, is_allied, is_others, tests(institute, series, title)').limit(100);
            ...
            if (sf.ncertFilter === 'NCERT Only') {
              fuzzyQry = fuzzyQry.eq('is_ncert', true);
            } else if (sf.ncertFilter === 'Non-NCERT') {
              fuzzyQry = fuzzyQry.or('is_ncert.is.null,is_ncert.eq.false');
            }
            ...
            if (fData && fData.length > 0) {
              ...
              fData.forEach(fd => {
                if (!existingIds.has(fd.id)) mergedData.push(fd);
              });
              data = mergedData;
            }
          }
      ...
      mergedQs.sort((a: any, b: any) => {
        ...
        const aExact = aText.includes(term);
        const bExact = bText.includes(term);
  ...
  const updateQuestionCount = async () => {
    ...
    try {
      ...
      if (activeTab === 'topic') {
        ...
        // Metadata Filters (Search Tab Only)
        if (sf.selectedSubjects?.length > 0) query = query.in('subject', sf.selectedSubjects);
        if (sf.selectedSections?.length > 0) {
          const sections = sf.selectedSections.map((s: string) => s === "General" ? null : s);
          if (sections.includes(null)) {
        ...
        // PYQ Filters (Search Tab Only)
        if (sf.pyqFilter === 'PYQ Only') {
          query = query.eq('is_pyq', true);
          if (sf.pyqCategory?.length > 0) {
            const cats = sf.pyqCategory;
            const orFilters = [];
            if (cats.includes('UPSC') || cats.includes('UPSC CSE')) orFilters.push('is_upsc_cse.eq.true');
            if (cats.includes('Allied')) orFilters.push('is_allied.eq.true');
            if (cats.includes('Others')) orFilters.push('is_others.eq.true');
            if (orFilters.length > 0) query = query.or(orFilters.join(','));
          }
        ...
        // Institute, Program & Stage Filters (Search Tab Only)
        ...
        if (term && session?.user?.id) {
            // Note search is handled by the main results query
        }
      ...
      if (sf.searchMode !== 'Exact' && activeTab === 'search' && searchQuery.trim().length > 3 && finalCount < 50) {
        ...
        if (words.length === 1) {
          ...
          for (let i = 0; i < word.length; i++) {
            ...
            if (activeFields.includes('Explanations')) {
              fuzzyPatterns.push(`explanation_markdown.ilike.%${pattern}%`);
            }
          }
          
          let fuzzyQ = supabase.from('questions').select('id', { count: 'exact', head: true }).or(fuzzyPatterns.join(','));
          // Apply same filters (simplified for count)
          ...
          const { count: fCount } = await fuzzyQ;
          if (fCount) finalCount = Math.max(finalCount, fCount); 
        }
      }
